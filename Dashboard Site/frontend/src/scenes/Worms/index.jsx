...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('Worms ErrorBoundary caught:', error, errorInfo);
...
const Worms = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!storedToken) {
        ...
        setError('Please log in to access the worms data');
        navigate('/retry');
        return;
      }
      ...
      if (metricsResponse.data.status === 'success' && metricsResponse.data.data) {
        ...
        if (!metricsResponse.data.worms || metricsResponse.data.worms.current === 0) {
          console.warn('No Worms metrics returned. Check backend recent_incidents.');
        }
      } else {
      ...
      console.log('Fetching /api/recent-incidents?scenario=worms...');
      const incidentsResponse = await axios.get('/api/recent-incidents', {
        params: { scenario: 'worms', page: 1, per_page: 50 },
        headers: { Authorization: `Bearer ${storedToken}` },
      });
      ...
      if (incidentsResponse.data.status === 'success' && Array.isArray(incidentsResponse.data.data)) {
        const wormsIncidents = incidentsResponse.data.data.filter(incident => 
          incident.attack_cat === 'Worms'
        );
        console.log('Filtered Worms incidents:', JSON.stringify(wormsIncidents, null, 2));
        setIncidents(wormsIncidents.length > 0 ? wormsIncidents : incidentsResponse.data.data);
        if (wormsIncidents.length === 0) {
          console.warn('No Worms incidents found in worms scenario. Showing all worms incidents. Check backend attack_cat values for exact casing (expected: "Worms").');
        }
      } else {
  ...
  if (!metrics) {
    return (
      <ErrorBoundary>
        ...
        >
          <Typography variant="h5">Loading Worms data...</Typography>
        </Box>
      </ErrorBoundary>
    );
  }

  const wormsData = metrics.worms || { current: 0 };
  ...
  return (
    <ErrorBoundary>
      ...
      >
        <Header title="WORMS DETECTION" subtitle="Monitor Worm Attacks" />
        <Box mb="40px">
          <Box mb="20px">
            ...
            <Typography variant="h6" color={colors.grey[100]}>
              Worms Incidents: {wormsData.current || 0} occurrence(s)
            </Typography>
            {wormsData.current === 0 && (
              <Typography variant="body1" color={colors.redAccent[500]}>
                Warning: No Worms incidents detected. Verify backend dataset contains attack_cat: Worms.
              </Typography>
            )}
          </Box>
          <Box>
            ...
            {incidents.length > 0 ? (
              ...
              <Typography color={colors.grey[100]}>
                No Worms incidents detected. Check backend data for scenario=worms and attack_cat: Worms.
              </Typography>
            )}
          </Box>
        </Box>
