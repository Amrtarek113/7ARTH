...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('DoS ErrorBoundary caught:', error, errorInfo);
...
const DoS = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!storedToken) {
        ...
        setError('Please log in to access the DoS data');
        return;
      }
      ...
      const incidentsResponse = await getRecentIncidents('iot', 1, 50);
      ...
      if (incidentsResponse.status === 'success' && Array.isArray(incidentsResponse.data)) {
        const dosIncidents = incidentsResponse.data.filter(incident => 
          incident.attack_cat === 'DoS'
        );
        console.log('Filtered DoS incidents:', JSON.stringify(dosIncidents, null, 2));
        setIncidents(dosIncidents.length > 0 ? dosIncidents : incidentsResponse.data);
        if (dosIncidents.length === 0) {
          console.warn('No DoS incidents found in iot scenario. Showing all iot incidents. Check backend attack_cat values for exact casing (expected: "DoS").');
        }
      } else {
  ...
  if (!metrics) {
    return (
      ...
      >
        <Typography variant="h5">Loading DoS data...</Typography>
      </Box>
    );
  }

  const dosData = metrics.iot_attacks || { current: 0 };
  ...
  return (
    <ErrorBoundary>
      ...
      >
        <Header title="DOS DETECTION" subtitle="Monitor Denial of Service Attacks" />
        <Box mb="40px">
          <Box mb="20px">
            ...
            <Typography variant="h6" color={colors.grey[100]}>
              DoS Incidents: {dosData.current || 0} occurrence(s)
            </Typography>
            {dosData.current === 0 && (
              <Typography variant="body1" color={colors.redAccent[500]}>
                Warning: No DoS incidents detected. Verify backend dataset contains attack_cat: DoS.
              </Typography>
            )}
          </Box>
          <Box>
            ...
            {incidents.length > 0 ? (
              ...
              <Typography color={colors.grey[100]}>
                No DoS incidents detected. Check backend data for scenario=iot and attack_cat: DoS.
              </Typography>
            )}
          </Box>
        </Box>
