...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('IoT ErrorBoundary caught:', error, errorInfo);
...
const IoT = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!storedToken) {
        ...
        setError('Please log in to access the IoT data');
        navigate('/login');
        return;
      }
      ...
      if (metricsResponse.data.status === 'success' && metricsResponse.data.data) {
        ...
        if (!metricsResponse.data.iot_attacks || metricsResponse.data.iot_attacks.current === 0) {
          console.warn('No IoT metrics returned. Check backend recent_incidents.');
        }
      } else {
      ...
      if (incidentsResponse.data.status === 'success' && Array.isArray(incidentsResponse.data.data)) {
        const iotIncidents = incidentsResponse.data.data.filter(incident => 
          ['DoS', 'Backdoor'].includes(incident.attack_cat)
        );
        console.log('Filtered IoT incidents:', JSON.stringify(iotIncidents, null, 2));
        setIncidents(iotIncidents.length > 0 ? iotIncidents : incidentsResponse.data.data);
        if (iotIncidents.length === 0) {
          console.warn('No DoS or Backdoor incidents found in iot scenario. Showing all iot incidents. Check backend attack_cat values for exact casing (expected: "DoS", "Backdoor").');
        }
      } else {
  ...
  if (!metrics) {
    return (
      ...
      >
        <Typography variant="h5">Loading IoT data...</Typography>
      </Box>
    );
  }

  const iotData = metrics.iot_attacks || { current: 0 };
  ...
  return (
    <ErrorBoundary>
      ...
      >
        <Header title="IOT DETECTION" subtitle="Monitor IoT Device Attacks" />
        <Box mb="40px">
          <Box mb="20px">
            ...
            <Typography variant="h6" color={colors.grey[100]}>
              IoT Incidents: {iotData.current || 0} occurrence(s)
            </Typography>
            {iotData.current === 0 && (
              <Typography variant="body1" color={colors.redAccent[500]}>
                Warning: No IoT incidents detected. Verify backend dataset contains attack_cat: DoS or Backdoor.
              </Typography>
            )}
          </Box>
          <Box>
            ...
            {incidents.length > 0 ? (
              ...
              <Typography color={colors.grey[100]}>
                No IoT incidents detected. Check backend data for scenario=iot and attack_cat: DoS or Backdoor.
              </Typography>
            )}
          </Box>
        </Box>
