...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('Backdoors ErrorBoundary caught:', error, errorInfo);
...
const Backdoors = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!storedToken) {
        ...
        setError('Please log in to access the backdoors data');
        navigate('/login');
        return;
      }
      ...
      if (metricsResponse.data.status === 'success' && metricsResponse.data.data) {
        ...
        if (!metricsResponse.data.backdoors || metricsResponse.data.backdoors.current === 0) {
          console.warn('No Backdoors metrics returned. Check backend recent_incidents.');
        }
      } else {
      ...
      console.log('Fetching /api/recent-incidents?scenario=iot...');
      const incidentsResponse = await axios.get('/api/recent-incidents', {
        params: { scenario: 'iot', page: 1, per_page: 50 },
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (incidentsResponse.data.status === 'success' && Array.isArray(incidentsResponse.data.data)) {
        const backdoorIncidents = incidentsResponse.data.data.filter(incident => 
          incident.attack_cat === 'Backdoor'
        );
        console.log('Filtered Backdoor incidents:', JSON.stringify(backdoorIncidents, null, 2));
        setIncidents(backdoorIncidents.length > 0 ? backdoorIncidents : incidentsResponse.data.data);
        if (backdoorIncidents.length === 0) {
          console.warn('No Backdoor incidents found in iot scenario. Showing all iot incidents. Check backend attack_cat values for exact casing (expected: "Backdoor").');
        }
      } else {
  ...
  if (!metrics) {
    return (
      ...
      >
        <Typography variant="h5">Loading Backdoors data...</Typography>
      </Box>
    );
  }

  const backdoorsData = metrics.backdoors || { current: 0 };
  ...
  return (
    <ErrorBoundary>
      ...
      >
        <Header title="BACKDOORS DETECTION" subtitle="Monitor Backdoor Attacks" />
        <Box mb="40px">
          <Box mb="20px">
            <Typography variant="h5" color={colors.grey[100]} fontWeight="600" mb="10px">
              Backdoors Statistics
            </Typography>
            <Typography variant="h6" color={colors.grey[100]}>
              Backdoors Incidents: {backdoorsData.current || 0} occurrence(s)
            </Typography>
            {backdoorsData.current === 0 && (
              <Typography variant="body1" color={colors.redAccent[500]}>
                Warning: No Backdoor incidents detected. Verify backend dataset contains attack_cat: Backdoor.
              </Typography>
            )}
          </Box>
          <Box>
            ...
            {incidents.length > 0 ? (
              ...
              <Typography color={colors.grey[100]}>
                No Backdoor incidents detected. Check backend data for scenario=iot and attack_cat: Backdoor.
              </Typography>
            )}
          </Box>
        </Box>
