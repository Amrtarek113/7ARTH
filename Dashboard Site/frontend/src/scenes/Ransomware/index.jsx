...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('Ransomware ErrorBoundary caught:', error, errorInfo);
...
const Ransomware = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!storedToken) {
        ...
        setError('Please log in to access the ransomware data');
        navigate('/login');
        return;
      }
      ...
      if (metricsResponse.data.status === 'success' && metricsResponse.data.data) {
        ...
        if (!metricsResponse.data.ransomware_incidents || metricsResponse.data.ransomware_incidents.current === 0) {
          console.warn('No Ransomware metrics returned. Check backend recent_incidents.');
        }
      } else {
      ...
      if (incidentsResponse.data.status === 'success' && Array.isArray(incidentsResponse.data.data)) {
        const ransomwareIncidents = incidentsResponse.data.data.filter(incident => 
          ['Exploits', 'Shellcode'].includes(incident.attack_cat)
        );
        console.log('Filtered Ransomware incidents:', JSON.stringify(ransomwareIncidents, null, 2));
        setIncidents(ransomwareIncidents.length > 0 ? ransomwareIncidents : incidentsResponse.data.data);
        if (ransomwareIncidents.length === 0) {
          console.warn('No Exploits or Shellcode incidents found in ransomware scenario. Showing all ransomware incidents. Check backend attack_cat values for exact casing (expected: "Exploits", "Shellcode").');
        }
      } else {
  ...
  if (!metrics) {
    return (
      ...
      >
        <Typography variant="h5">Loading Ransomware data...</Typography>
      </Box>
    );
  }

  const ransomwareData = metrics.ransomware_incidents || { current: 0 };
  ...
  return (
    <ErrorBoundary>
      ...
      >
        <Header title="RANSOMWARE DETECTION" subtitle="Monitor Ransomware Attacks" />
        <Box mb="40px">
          <Box mb="20px">
            ...
            <Typography variant="h6" color={colors.grey[100]}>
              Ransomware Incidents: {ransomwareData.current || 0} occurrence(s)
            </Typography>
            {ransomwareData.current === 0 && (
              <Typography variant="body1" color={colors.redAccent[500]}>
                Warning: No Ransomware incidents detected. Verify backend dataset contains attack_cat: Exploits or Shellcode.
              </Typography>
            )}
          </Box>
          <Box>
            ...
            {incidents.length > 0 ? (
              ...
              <Typography color={colors.grey[100]}>
                No Ransomware incidents detected. Check backend data for scenario=ransomware and attack_cat: Exploits or Shellcode.
              </Typography>
            )}
          </Box>
        </Box>
