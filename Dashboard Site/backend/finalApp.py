...
# pyjwt Configuration
app.config['pyjwt_SECRET_KEY'] = os.getenv('pyjwt_SECRET_KEY', secrets.token_hex(32))
app.config['pyjwt_ACCESS_TOKEN_EXPIRES'] = timedelta(hours=24)
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            data = pyjwt.decode(token, app.config['pyjwt_SECRET_KEY'], algorithms=['HS256'])
...
def login():
    ...
    try:
        ...
        token_payload = {
            ...
            'exp': datetime.utcnow() + app.config['pyjwt_ACCESS_TOKEN_EXPIRES']
        }
        token = pyjwt.encode(token_payload, app.config['pyjwt_SECRET_KEY'], algorithm='HS256')
...
def analyze_uploaded_file():
    """
    Analyze an uploaded file (PCAP or CSV) and return the analysis result.
    Supports JSON, form-data, and query param.
    """
    filename = None
    ...
    # 4. من اسم الملف نفسه إذا تم رفعه مباشرة
    ...
    try:
        ...
        logger.error(f"Analyze file error: {str(e)}")
        return jsonify({
            'status': 'error',
...
def upload_file():
    if 'file' not in request.files:
        return jsonify({'status': 'error', 'message': 'No file part', 'error_code': 'NO_FILE'}), 400

    file = request.files['file']
    if file.filename == '':
        return jsonify({'status': 'error', 'message': 'No selected file', 'error_code': 'NO_FILENAME'}), 400
    ...
    return jsonify({'status': 'success', 'message': f'{filename} uploaded successfully', 'filename': filename})
