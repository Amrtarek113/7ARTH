...
# Mock IP and MAC address pools with associated locations
mock_ip_locations = {
    "192.168.1.10": {"country": "USA", "city": "New York"},
    "192.168.1.11": {"country": "Germany", "city": "Berlin"},
    "10.0.0.5": {"country": "Japan", "city": "Tokyo"},
    "172.16.0.20": {"country": "UK", "city": "London"},
    "192.168.0.15": {"country": "France", "city": "Paris"},
    "203.0.113.1": {"country": "Canada", "city": "Toronto"},
    "198.51.100.25": {"country": "Australia", "city": "Sydney"},
    "192.0.2.30": {"country": "Brazil", "city": "Rio de Janeiro"},
    "172.217.1.100": {"country": "India", "city": "Mumbai"},
    "209.85.200.50": {"country": "China", "city": "Beijing"}
}
mock_ips = list(mock_ip_locations.keys())
...
def autonomous_detection():
    ...
    while True:
        ...
        try:
            ...
            # Get a random IP and its associated location
            selected_ip = random.choice(mock_ips)
            ip_location = mock_ip_locations.get(selected_ip, {"country": "Unknown", "city": "Unknown"})
            ...
            incident = {
                ...
                "src_ip": sample.get('src_ip', selected_ip),
                "mac_address": sample.get('mac_address', random.choice(mock_macs)),
                "src_country": ip_location["country"],
                "src_city": ip_location["city"]
            }
            ...
            # Log detection
            logger.debug(f"Detected {attack_cat} with confidence {prob:.4f}, src_ip: {incident['src_ip']}, mac_address: {incident['mac_address']}, location: {incident['src_city']}, {incident['src_country']}")
...
def get_recent_incidents():
    ...
    src_country = request.args.get('src_country') # Added for country filtering
    src_city = request.args.get('src_city') # Added for city filtering
    page = int(request.args.get('page', 1))
    per_page = int(request.args.get('per_page', 10))
    
    # Filter incidents by scenario, src_ip, mac_address, country, and city
    with incidents_lock:
        ...
        if src_country:
            filtered_incidents = [incident for incident in filtered_incidents if incident.get('src_country') and incident['src_country'].lower() == src_country.lower()]
        if src_city:
            filtered_incidents = [incident for incident in filtered_incidents if incident.get('src_city') and incident['src_city'].lower() == src_city.lower()]
...
def get_unique_locations():
    """Get unique countries and cities from recent incidents."""
    unique_countries = set()
    unique_cities = set()
    
    with incidents_lock:
        for incident in recent_incidents:
            if 'src_country' in incident and incident['src_country']:
                unique_countries.add(incident['src_country'])
            if 'src_city' in incident and incident['src_city']:
                unique_cities.add(incident['src_city'])
