...
try:
    ...
    analyze_file = None
    logger.warning("pyshark not installed, file analysis disabled")
...
def autonomous_detection():
    ...
    while True:
        ...
        try:
            ...
            # Extract features (drop only columns that exist)
            drop_cols = [col for col in ['id', 'attack_cat', 'label', 'src_ip', 'mac_address'] if col in sample.index]
            features = sample.drop(drop_cols).to_dict()
...
@permission_required('read')
def geoip_lookup():
    """Handle GeoIP lookup with robust IP validation."""
    try:
        # Parse JSON body
        data = request.get_json(silent=True)
        if not data or not isinstance(data, dict):
            logger.error("Invalid JSON body received: %s", request.data.decode('utf-8', errors='ignore'))
            return jsonify({
                'status': 'error',
        ...
        # Validate IP address
        ip = data.get('ip')
        if not ip or not isinstance(ip, str) or ip.strip() == '':
            logger.error("Missing or empty IP address: %s", ip)
            return jsonify({
                'status': 'error',
                'message': 'IP address is required and must be a non-empty string',
        ...
        if not re.match(ip_regex, ip.strip()):
        ...
        result = get_location_from_ip(ip.strip())
        
        if 'error' in result:
            logger.error("GeoIP lookup failed for %s: %s", ip, result['error'])
            return jsonify({
                'status': 'error',
                'error_code': 'GEOIP_LOOKUP_ERROR'
            }), 500
        ...
        logger.error("Unexpected error in geoip_lookup for IP %s: %s", ip or 'None', str(e))
        return jsonify({
            'status': 'error',
